# Graph Report - Anvesha-'26  (2026-08-12)

## Corpus Check
- 18 files · ~20,248 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 169 nodes · 218 edges · 15 communities (13 shown, 2 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 2 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `eebc83dd`
...
3. `lucide-static/icons/arrow-right.svg?raw` - 6 edges
4. `scripts` - 5 edges
5. `syncTree()` - 5 edges
6. `lucide-static/icons/flask-conical.svg?raw` - 4 edges
7. `renderViews()` - 4 edges
8. `syncViews()` - 4 edges
9. `goView()` - 4 edges
10. `renderMGrid()` - 4 edges

## Surprising Connections (you probably didn't know these)
- `renderMGrid()` --calls--> `selectItem()`  [EXTRACTED]
  src/pages/merch.astro → src/pages/merch.astro  _Bridges community 4 → community 14_
...
## Knowledge Gaps
- **89 isolated node(s):** `name`, `type`, `version`, `node`, `dev` (+84 more)
...
- **Why does `lucide-static/icons/flask-conical.svg?raw` connect `../layouts/SiteLayout.astro` to `events.astro`, `merch.astro`?**
